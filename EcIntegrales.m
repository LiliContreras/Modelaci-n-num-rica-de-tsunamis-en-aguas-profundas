...
rt=50; %50 segundos retraso de tiempo
...
for i=1:length(f)
    ...
    if F==1
	    desp=disprig;
    elseif F==2
        disp=[zeros(1,rt),ones(1,length(f)-rt)];
...
%FRECUENCIA SOLO PARTE POSITIVA SOLO SE USA PARA VISUALIZACIÓN
DDP=zeros(length(f)+1,length(obs.xO));
DDP(1,:)=real(un0.dispn(:,1));
for j=1:length(obs.xO)
    for i=2:length(DDP(:,1))
        DDP(i,j)=un(i-1).dispn(j);
    end
end
...
%FILTRO PASA ALTAS
FC=0.205; FI=0.125
FPA=zeros(1,length(frec));
for i=1:length(frec)
    if frec(i)<FC && frec(i)>FI
        FPA(1,i)=1;
    end
end
FPAN=fliplr(FPA);
...
for i=1:length(obs.xO)
    for j=1:length(f)*2
        DDR(j,i)=DD(j,i)*FPAGC(j)*W(j);
    end
end
%{
%FIGURA DE ESPECTROS DE AMPLITUD
figure
%fn=linspace(-frfinal,frfinal,length(DDR(:,i)));
for i=1:length(obs.xO)
    subplot(3,1,1)
    plot(f,abs(DD(1:127,i)))
    ylim([0 6])
    ylabel('Amplitud')
    subplot(3,1,2)
    plot(f,FPAGC(1:127))
...
ylabel('Distancia [m]','FontSize',13)
if C==0
...
yticks([0:20:100])
yticklabels({num2str(obs.xO(1)),num2str(obs.xO(20)),num2str(obs.xO(40)),num2str(obs.xO(60)),num2str(obs.xO(80)),num2str(obs.xO(100))})
...
%-----------------------------------------------------------
%COMPARACIÓN CONTRIBUCIONES DE ESPECTRO DE AMPLITUD
FIV=linspace(0.01,0.18,20);
if C==0
    rutaFIG=['/home/lcontrerasal/Documentos/Tesis/Imagenes/Metodo_Imagenes/Propagacion_Espectros_FIltro/'];
elseif C==1
    rutaFIG=['/home/lcontrerasal/Documentos/Tesis/Imagenes/Metodo_P0/Propagacion_Espectro_Filtro'];
else 
    rutaFIG=['/home/lcontrerasal/Documentos/Tesis/Imagenes/Metodo_Gravedad/Propagacion_Espectro_Filtro'];
end

for i=1:length(FIV)+1
    
%FILTRO PASA ALTAS
FC=0.205; 
FI=FIV(length(FIV)-i);
titulo=['FrecuenciaMin' num2str(FI)];
FPA=zeros(1,length(frec));
for j=1:length(frec)
    if frec(j)<FC && frec(j)>FI
        FPA(1,j)=1;
    end
end
FPAN=fliplr(FPA);
...
%CONVOLUCIÓN CON ONDÍCULA
for k=1:length(obs.xO)
    for l=1:length(f)*2
        DDR(l,k)=DD(l,k)*FPAGC(l);
    end
end


%TRANSFORMANDO A DOMINIO DEL TIEMPO
for m=1:length(obs.xO)
    dd(:,m)=real(ifft(DDR(:,m)));
end
...
figure('Position',[1 1 scrsz(3) scrsz(4)]);
subplot(3,2,[1,3,5])
for n=1:length(obs.xO)
    plot(t,dd(:,n)/max(dd(:,n))+n,'k')
    hold on
end
xlabel('Tiempo [s]', 'FontSize',13)
ylabel('Distancia [m]','FontSize',13)
...
yticks([0:20:100])
yticklabels({num2str(obs.xO(1)),num2str(obs.xO(20)),num2str(obs.xO(40)),num2str(obs.xO(60)),num2str(obs.xO(80)),num2str(obs.xO(100))})
%Recta de velocida y=mx + b
%b=obs.xO(59);
b=58;
m=1/c;
funcionV=t*m + b;
hold on
plot(t(1:44),funcionV(1:44),'r')
%text(10,90,'$\rightarrow V=1500$','Interpreter','latex')
text(56,103,'$\leftarrow V=1500 \frac{m}{s}$','Interpreter','latex')
ylim([0 105])
hold on
    subplot(3,2,2)
    plot(f,abs(DD(1:127,10)))
    ylim([0 6])
    ylabel('Amplitud')
    subplot(3,2,4)
    plot(f,FPAGC(1:127))
    ...
    ylabel('Amplitud')
    subplot(3,2,6)
    plot(f,abs(DDR(1:127,10)))
    ylim([0 6])
    xlabel('Frecuencia')
    ylabel('Amplitud')
    pause(0.3)
eval(['print -dpng ' rutaFIG titulo '_1']);
end
%-----------------------------------------------------------
...
frec=linspace(-frfinal,frfinal,length(f)*2);
figure
for i=1:length(obs.xO)
    plot(frec,abs(DD(:,i)))
    ylim([0 10])
...
fc = 0.2; % Frecuencia de corte en Hz
fs=2*frfinal;
% Diseña el filtro
[b, a] = butter(order, fc/(fs/2), 'low');
for i=1:length(obs.xO)
    filtered_signal(:,i) = filter(b, a, DD(:,i));
end
